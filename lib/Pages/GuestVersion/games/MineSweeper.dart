  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      child: Container(
        ...
        child: Center(
          child: (widget.mine && widget.covered == false)
              ? Icon(
                  Icons.local_fire_department_outlined,
                  color: Colors.red,
                )
              : Text(
                  widget.val == 0 || widget.covered
                      ? ''
                      : widget.val.toString(),
                  style: TextStyle(fontSize: 23),
                ), //Testing (reveals all mines)     (widget.mine) ? Icon(Icons.local_fire_department_outlined, color: Colors.red,) : Text( widget.val == 0 || widget.covered ? '' : widget.val.toString(), style: TextStyle(fontSize: 23),),
        ),
      ),
      onTap: () {
